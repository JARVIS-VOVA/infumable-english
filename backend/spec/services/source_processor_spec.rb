...
RSpec.describe SourceProcessor do
  describe '#call' do
    ...
    it 'creates new terms and increments priorities for existing terms' do
      source.terms.create!(user: user, phrase: 'hello', meaning: nil, priority: 1)

      SourceProcessor.new(text: 'Hello world hello', source_id: source.id).call

      expect(source.terms.count).to eq(2)
      expect(source.terms.find_by(phrase: 'hello')&.priority).to eq(3)
      expect(source.terms.find_by(phrase: 'world')&.priority).to eq(1)

      SourceProcessor.new(text: 'hello again', source_id: source.id).call
    ...
    it 'preserves existing meanings' do
      ...
      SourceProcessor.new(text: 'hello world', source_id: source.id).call
