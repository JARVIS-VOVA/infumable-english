...
export const TermsList: React.FC = () => {
  ...
  const { data: termsResponse, isLoading, error } = useTerms({ page, perPage: PAGE_SIZE })
  ...
  const currentPage = termsResponse?.meta?.currentPage || page
  const totalTermsCount = termsResponse?.meta?.totalCount || 0
